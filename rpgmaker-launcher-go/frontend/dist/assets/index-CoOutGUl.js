...
Place .zip files next to the launcher and click Refresh.`,dragDropZip:"Drop .zip files here to add them!",navPlay:"Library",navPlugins:"Plugins",navSaves:"Saves",navData:"Data",navMods:"Mods",navSync:"Sync",navDecrypt:"Decrypt",navShortcuts:"Shortcuts",navQuit:"Quit",navSettings:"Settings",btnPlay:"Play",btnStopServer:"Stop Server",btnRefresh:"Refresh",btnSave:"Save Changes",btnCancel:"Cancel",btnClose:"Close",btnBackup:"Backup",btnRestore:"Restore",btnExport:"Export",btnDelete:"Delete",btnEnableAll:"All ON",btnDisableAll:"All OFF",btnPush:"Push to Destination →",btnPull:"← Pull from Destination",btnChangeFolder:"Change...",btnOpenFolder:"Open Folder",toggleWebKit:"WebKit",toggleDelZip:"Delete .zip",toggleWebKitTip:"Use the WebKit viewer (lighter) instead of the browser",toggleDelZipTip:"Delete the .zip after extracting",updateChipTip:"New version available. Click to view releases.",modsReadyToast:"Mods folder ready and opened. Every .js is injected on launch; reload with F5.",quitTip:"Close the launcher",settingsTitle:"Settings",settingsDesc:"Choose the folder where games and .zip files will be stored.",settingsGamesFolder:"Games folder:",settingsDefaultFolder:"Default: %s",playedHours:"played",playedNow:"Played just now",playedMin:"Played %d min ago",playedHoursAgo:"Played %d h ago",playedDaysAgo:"Played %d d ago",neverPlayed:"Not played yet",incompleteBadge:"Incomplete",incompleteNotice:"Incomplete download (missing files)",serverActive:"Active Server:",serverPort:"Port",serverStopped:"Server stopped",runtimeReady:"RPG Maker Engine Runtime v{version}",pluginsTitle:"Plugins",pluginsDesc:"Manage and analyze WebKit / nw.js compatibility",savesTitle:"Save Manager",savesDesc:"Backups, snapshots, and save editing",saveEditorTitle:"Save Editor",saveEditorWarning:"⚠ Close the game before editing: autosaves might overwrite your changes.",saveEditorGold:"Party Gold",saveEditorItems:"Items",saveEditorVariables:"Variables",saveEditorSwitches:"Switches",saveEditorGeneral:"General",dataTitle:"Database Browser",syncTitle:"Save Sync",syncDestFolder:"Destination folder:",syncAutoToggle:"Automatically sync saves when closing a game",decryptTitle:"Decryption Tool",decryptNotice:"Decrypts .rgssad/.rgss2a/.rgss3a and RPG Maker MV/MZ encrypted assets.",shortcutsTitle:"Shortcuts & Preferences",toastServerStarted:"Server started on port %d",toastServerStopped:"Server stopped (%d sec played)",toastSaved:"Save file updated successfully",toastSyncDone:"Sync completed",toastBackupDone:"Backup created"}};let v="es";function k(){return v}function y(u){v=u,document.documentElement.lang=u}function o(u,...e){let a=(f[v]||f.es)[u]||f.es[u]||u;return e.length>0&&e.forEach(r=>{a=a.replace(/%[ds]/,String(r))}),a}class q{constructor(e){c(this,"activeTab","library");c(this,"callbacks");this.callbacks=e}setActiveTab(e){this.activeTab=e,this.render()}render(){const e=document.createElement("aside");return e.className="fixed left-0 top-0 h-full w-60 bg-surface-container-lowest z-30 flex flex-col border-r border-border select-none",e.innerHTML=`
    ...
    `}render(){var i,l,d,n,h;const e=document.createElement("header");e.className="h-14 shrink-0 bg-surface border-b border-border flex items-center justify-between gap-3 px-6 select-none";const t=k();e.innerHTML=`
    ...
    `,document.body.appendChild(this.modalEl),this.modalEl.querySelectorAll(".btn-close").forEach(r=>{r.addEventListener("click",()=>this.close())}),(e=this.modalEl.querySelector("#btn-toggle-all-on"))==null||e.addEventListener("click",()=>this.toggleAll(!0)),(t=this.modalEl.querySelector("#btn-toggle-all-off"))==null||t.addEventListener("click",()=>this.toggleAll(!1)),(a=this.modalEl.querySelector("#btn-restore-plugins"))==null||a.addEventListener("click",()=>this.restoreOriginal()),await this.loadData()}async loadData(){var e;try{const t=await b.getPlugins(this.game.name);this.plugins=(t==null?void 0:t.plugins)||[],this.hasBackup=(t==null?void 0:t.has_backup)||!1,this.renderTable()}catch(t){const a=(e=this.modalEl)==null?void 0:e.querySelector("#plugins-table-container");a&&(a.innerHTML=`<div class="p-6 text-center text-status-error">Error al cargar plugins: ${t.message}</div>`)}}renderTable(){var a;const e=(a=this.modalEl)==null?void 0:a.querySelector("#plugins-table-container");if(!e)return;if(this.plugins.length===0){e.innerHTML='<div class="p-6 text-center text-text-muted">No se encontraron plugins en este juego.</div>';return}if(!this.plugins||this.plugins.length===0){e.innerHTML='<div class="p-6 text-center text-text-muted">No se encontraron plugins en este juego.</div>';return}const t=this.plugins.map((r,s)=>{let i="bg-status-success/15 text-status-success",l="OK";return r.category==="nw-protegido"?(i="bg-status-warning/15 text-status-warning",l="NW PROTECTED"):r.category==="roto"?(i="bg-status-error/15 text-status-error",l="BROKEN"):r.category==="sin-fichero"&&(i="bg-surface-variant text-text-faint",l="MISSING"),`
    ...
    `,e.querySelectorAll(".save-row").forEach(r=>{r.addEventListener("click",s=>{const i=s.currentTarget.dataset.name;this.selectedSave=this.saves.find(l=>l.name===i)||null,this.renderTable(),this.updateEditButtonState()}),r.addEventListener("dblclick",()=>{this.openEditor()})})}async createBackup(){try{const e=await b.backupSaves(this.game.name);p.show(`Copia de seguridad creada en snapshot-${e.timestamp}`,"success")}catch(e){p.show(`Error creando copia: ${e.message}`,"error")}}openEditor(){if(!this.selectedSave)return;new z(this.game,this.selectedSave.name).open()}close(){var e;(e=this.modalEl)==null||e.remove(),this.modalEl=null}}class N{constructor(e){c(this,"game");c(this,"currentCategory","Items");c(this,"items",[]);c(this,"searchQuery","");c(this,"modalEl",null);this.game=e}async open(){this.modalEl=document.createElement("div"),this.modalEl.className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 select-none",this.modalEl.innerHTML=`
    ...
    `}close(){var e;(e=this.modalEl)==null||e.remove(),this.modalEl=null}}async function B(){return new Promise(u=>{var a,r;const e=document.createElement("div");e.className="fixed inset-0 z-[60] flex items-center justify-center bg-background/80",e.innerHTML=`
      <div class="bg-surface-container border border-border rounded-xl shadow-2xl p-6 w-[420px]">
        <h3 class="text-headline-sm font-bold text-on-surface mb-3">Carpeta de destino</h3>
        <p class="text-body-sm text-text-muted mb-4">Introduce la ruta de la carpeta de destino para sincronizar partidas:</p>
        <input id="folder-path-input" type="text" class="w-full px-3 py-2 bg-surface border border-border rounded-lg text-on-surface font-mono text-sm focus:outline-none focus:border-primary" placeholder="/home/usuario/Dropbox/Saves" autofocus />
    ...
    `,document.body.appendChild(e);const t=e.querySelector("#folder-path-input");t.focus(),t.addEventListener("keydown",s=>{s.key==="Enter"&&(u(t.value.trim()||null),e.remove()),s.key==="Escape"&&(u(null),e.remove())}),(a=e.querySelector("#folder-ok"))==null||a.addEventListener("click",()=>{u(t.value.trim()||null),e.remove()}),(r=e.querySelector("#folder-cancel"))==null||r.addEventListener("click",()=>{u(null),e.remove()}),e.addEventListener("click",s=>{s.target===e&&(u(null),e.remove())})})}class F{constructor(){c(this,"syncData",null);c(this,"modalEl",null);c(this,"folder","")}async open(){var a,r,s,i;this.modalEl=document.createElement("div"),this.modalEl.className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 select-none",this.modalEl.innerHTML=`
...
ERROR: ${i.message}`),p.show(`Error descifrando: ${i.message}`,"error")}}close(){var e;(e=this.modalEl)==null||e.remove(),this.modalEl=null}}class I{constructor(){c(this,"config",null);c(this,"modalEl",null)}async open(){var e;this.modalEl=document.createElement("div"),this.modalEl.className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 select-none",this.modalEl.innerHTML=`
    ...
    `,document.body.appendChild(e);const t=e.querySelector("#folder-path-input");t.focus(),t.addEventListener("keydown",s=>{s.key==="Enter"&&(u(t.value.trim()||null),e.remove()),s.key==="Escape"&&(u(null),e.remove())}),(a=e.querySelector("#folder-ok"))==null||a.addEventListener("click",()=>{u(t.value.trim()||null),e.remove()}),(r=e.querySelector("#folder-cancel"))==null||r.addEventListener("click",()=>{u(null),e.remove()}),e.addEventListener("click",s=>{s.target===e&&(u(null),e.remove())})})}class Z{constructor(){c(this,"folder","");c(this,"modalEl",null)}async open(){var t,a;this.modalEl=document.createElement("div"),this.modalEl.className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 select-none",this.modalEl.innerHTML=`
