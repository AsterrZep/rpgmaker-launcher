...
// Diálogo nativo de carpetas (solo disponible dentro de Tauri)
const isTauri = Boolean((window as any).__TAURI_INTERNALS__);
async function pickFolder(): Promise<string | null> {
  const { open } = await import('@tauri-apps/plugin-dialog');
  const sel = (await open({ directory: true, multiple: false, title: 'Carpeta de destino de partidas' })) as unknown as string | string[] | null;
