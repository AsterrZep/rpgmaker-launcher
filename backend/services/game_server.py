...
BRIDGE_PATH = os.path.join(BACKEND_DIR, "scripts", "savebridge.js")
...
def _config():
    ...
    spec = importlib.util.spec_from_file_location(
        "rpgmaker_config", os.path.join(BACKEND_DIR, "core", "config.py"))
...
class GameHandler(SimpleHTTPRequestHandler):
    ...
    def _serve_static_js(self, filename):
        path = os.path.join(BACKEND_DIR, "scripts", filename)
        try:
            with open(path, "rb") as fh:
                data = fh.read()
        except OSError:
