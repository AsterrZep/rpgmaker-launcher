...
SHIM_FILE = os.path.join(BACKEND_DIR, "runtimes", "win32-shim.rb")
...
try:
    mod_config = _load_module("core/config.py", "rpgmaker_config")
except Exception:
    mod_config = None

try:
    mod_plugins = _load_module("engine/plugins.py", "rpgmaker_plugins")
except Exception:
    mod_plugins = None

try:
    mod_saveedit = _load_module("engine/save_editor.py", "rpgmaker_saveedit")
except Exception:
    mod_saveedit = None

try:
    mod_sync = _load_module("services/sync.py", "rpgmaker_sync")
except Exception:
    mod_sync = None
...
RENPY_LIB_DIRS = [
    # Ren'Py 8.x (Python 3) — modern versions
    "py3-linux-x86_64",
    "py3-linux-i686",
    "py3-linux-aarch64",    # ARM64: Steam Deck, Raspberry Pi, Chromebooks
    # Ren'Py 7.x (Python 2) — legacy versions
    "py2-linux-x86_64",
    "py2-linux-i686",
    "py2-linux-aarch64",
    # Very old Ren'Py (< 7.x)
...
def _renpy_lib_ok(rdir):
    """Check that a Ren'Py game has native Linux libraries."""
...
class ActiveSession:
    ...
    def launch_web(self, name, root, webkit=False):
        ...
        server_py = os.path.join(BACKEND_DIR, "services", "game_server.py")
        ...
        if webkit:
            viewer_py = os.path.join(BACKEND_DIR, "services", "webview.py")
...
def find_mkxpz():
    ...
    cands += [
        ...
        os.path.join(BACKEND_DIR, "runtimes", "mkxp-z"),
...
class ApiHandler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == "/api/decrypt":
            ...
            game_top = os.path.join(games_dir, game)
            root, engine = detect_engine(game_top)
            decrypter_py = os.path.join(BACKEND_DIR, "engine", "decrypter.py")
